...
import com.lambert.front.service.CartService;
...
public class CartController {
    ...
    public ModelAndView show(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            if (user != null) {
                List<Cart> cartList = cartService.queryCartList(user);
    ...
    public String addProductToCart(@PathVariable("productId") Long productId, HttpServletRequest request, HttpServletResponse response) {
        User user = UserThreadLocal.get();
        try {
            if (user != null) {
                cartService.addProductToCart(user, productId);
            } else {
                cookieCartService.addProductToCart(request, response, productId);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public ResponseEntity<Boolean> updateNum(@PathVariable("productId") Long productId, @PathVariable("num") Long num, HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            if (user != null) {
                cartService.updateNum(user.getId(), productId, num);
